...
def check_license(opener, nsfeature):
    result = opener.get(resource='nslicense')
    logger.info('result nslicense: {}'.format(result))
    if result['nitro_errorcode'] == 258:
        return False
    elif result['nitro_errorcode'] == 0:
        nsfeaturelist = result['data']['nslicense']
        if nsfeaturelist[nsfeature]:
            return True
    else:
        logger.info('ha_node_exists result %s' % result)
        raise Exception('Failure to get ha node')
...
def add_appfw_profile(opener, name, configdict={}):
    post_data = {
        "appfwprofile": {
            "name": name,
        }
    }
    post_data['appfwprofile'].update(configdict)

    result = opener.post(resource='appfwprofile', post_data=post_data)
    logger.debug('add_appfw_profile result {}'.format(result))
    if result['nitro_errorcode'] == 0 or result['nitro_errorcode'] is None:
        logger.info('SUCCESS: AppFw Profile {} added for {}'.format(name, opener.nsip))
    else:
        raise Exception(
            'FAIL: Could not add AppFw Profile {} for {}'.format(name, opener.nsip))


def add_appfw_policy(opener, name, profilename, rule):
    post_data = {
        "appfwpolicy": {
            "name": name,
    ...
    result = opener.post(resource='appfwpolicy', post_data=post_data)
    logger.debug('add_appfw_policy result {}'.format(result))
    if result['nitro_errorcode'] == 0 or result['nitro_errorcode'] is None:
        logger.info('SUCCESS: AppFw Policy {} added for {}'.format(name, opener.nsip))
    else:
        raise Exception(
            'FAIL: Could not add AppFw Policy {} for {}'.format(name, opener.nsip))


def bind_appfw_global_policy(opener, policyname, priority, type):
    put_data = {
    ...
    logger.debug('bind_appfw_global_policy result: %s' % result)
    if result['nitro_errorcode'] == 0:
        logger.info('SUCCESS: AppFw policy {} is bound GLOBALLY in {}'.format(
            policyname, opener.nsip))
    else:
    ...
    try:
        if request_type == 'Create':
            ...
            try:  # If any of the following step fails, raise exception and return FAILED
                ...
                isAppfwLicensed = check_license(primary, nsfeature='appfw')
                ...
                configure_features(primary, ['AppFw'])
                ...
                add_appfw_policy(primary, name=APPFW_POLICY, profilename=APPFW_PROFILE, rule="true")
                bind_appfw_global_policy(primary, policyname=APPFW_POLICY, priority=100, type="REQ_DEFAULT")
